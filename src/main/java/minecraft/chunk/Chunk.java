...
abstract public class Chunk implements IChunk {
    ...
    public Chunk(int xOffset, int zOffset) {
       ...
       // setInitialBlocks();
        ...
        {
           addSubChunk(new SubChunk(xOffset, i * IChunk.SUBCHUNK_HEIGHT, zOffset));
        }
    ...
    public boolean isAir(int x, int y, int z) {
        SubChunk chunk = getSubChunk(this.offsetX, y % SUBCHUNK_HEIGHT, this.offsetZ);
        if (chunk == null) return true;
        return chunk.getChunkBlock(x, y, z).isType(Material.AIR);
    }

    @Override
    public ChunkBlock getChunkBlock(int x, int y, int z) {
        SubChunk chunk = getSubChunk(this.offsetX, y % SUBCHUNK_HEIGHT, this.offsetZ);
        if (chunk == null) return new ChunkBlock(Material.AIR);
        return chunk.getChunkBlock(x, y, z);
    ...
    public void setChunkBlock(int x, int y, int z, Material type) {
        ...
        chunk.setChunkBlock(x, y, z, type);
    }
