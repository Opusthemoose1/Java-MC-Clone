...
public class MoveCommand implements ICommand {
    ...
    public MoveCommand(Player player, Camera camera, Camera.CameraDirection direction)
    {
        ...
        IVector cameraSpawn = player.getLocation().toVector();
        cameraSpawn.setY(cameraSpawn.getY() + 1);
        camera.setPosition(cameraSpawn);
    }
    ...
    public void execute(float deltaTime)
    {
        ...
        final Vector position = (Vector) camera.getPosition().clone();
